...
`components/layout/app-shell.tsx` and `components/layout/topbar.tsx` — the one sanctioned edit is a one-line `<NewOverlay />` mount (app-shell) or a one-line indicator mount in the icon cluster (topbar); everything else in those files stays off-limits. `components/layout/sidebar.tsx` — add nav items only. Precedent already in the codebase: `ScreenProjectPiPOverlay`, `DriveThruOverlay`, `FloatingDebriefButton`, `AnnouncementOnLoadPopup`, `SoundFxInit`, `BottomNav` (mounted in app-shell), `DriveThruButton`, `TopbarToolsCluster`, `SoundToggle` (mounted in topbar). `components/layout/topbar-tools-cluster.tsx` is the collapsible topbar drawer holding `BreakTimerButton` + `StorePassportButton`; it forces itself open while a break runs. `BreakTimerButton` is a thin trigger over the ToolboxPizza Breaks API (server is the source of truth — the old localStorage timer is gone) and is also the single owner of break polling (`useBreaksSync`).

**Extension (safe to add to):**
`app/[locale]/(dashboard)/dashboard/**/page.tsx`, `components/widgets/**`, `components/features/**`, `components/dashboard-v1/**` (Dashboard V1 re-skin, reuses DSPR's hooks/services), `components/dspr/**` (DSPR dashboard — what Dashboard V1 re-skins), `components/screen-project/**` (incl. `drive-thru/**`), `components/cleaning/**` (Cleaning Chart feature — task scheduling/completion, evaluation grid, reports; tab-level access gated by `lib/auth/cleaning-access.ts`, not a feature flag), `components/scheduling/**` (weekly scheduling grid, wired to the OperationsPizza API; `day-view.tsx` and `month-overview.tsx` are PARKED and unmounted), `lib/scheduling/**` (scheduling constants, week identity/date helpers, API adapters, error parsing), `components/uisfx/**` (sound-fx UI: global click-sound + unlock singleton, topbar mute toggle), `lib/uisfx/**` (sound-fx logic: persisted preference store, lazy `uisfx` client singleton, store→library sync, `playSfx(cue)` call-site helper — gated by the `soundFx` feature flag), `lib/api/services/**`, `lib/store/**` (feature-scoped Zustand stores — distinct from the Core `lib/dashboard/store/**`. Two of them, `issue-basket.store.ts` and `pay-basket.store.ts`, are persisted WORKING SETS rather than caches of server state: issues picked up across tickets to act on together, and work marked for payment before a sheet exists. They live in localStorage for the same reason `use-ticket-draft` does — one coordinator's in-progress collection, free to keep, not shared state), `lib/notifications/**` (notification → page-segment routing, shared by the bell/panel click-routing and the sidebar unread-dot indicator), `lib/nav/**` (bottom-nav-eligible items list + permission filtering, mirrors `sidebar.tsx`'s nav metadata/order independently rather than importing from that Core file), `components/daily-pay/**` (Daily Pay v2 — three-level Entry -> Payment -> Line pay sheets), `lib/daily-pay/**` (Daily Pay pure logic: form state machine, money/hours formulas, 422 field-error mapping, aggregation-warning copy, v1/v2 revision-snapshot parsing, `explain.ts` — the plain-words breakdown that makes the four money traps visible — and `from-basket.ts`, which regroups the pay basket into a sheet; no React, no JSX), `lib/maintenance-tickets/**` (ticket-side pure logic: attendance duration parsing/formatting and the FORM-ONLY net-hours preview — the read-only card always renders the server's own `durations`; the action catalogue in `issue-actions.ts` that decides what can be done to an issue and what to say when it can't; `attendance-timeline.ts`, the event model behind the timeline entry; `corrections.ts`, which reads a record back into the form that made it; `filters-url.ts`, the filters ⇄ query-string round trip), `components/storage/**` (Storage & Stock — GLOBAL storage locations, the append-only stock-movement ledger, and on-hand balances; NOT store-scoped), `lib/storage/**` (stock pure logic: the movement-type/direction table, composer payload builder, mistaken→reversal pairing, and `stock-actions.ts` — the plain-language layer that maps what people say happened onto the seven movement types, so the jargon never reaches the screen), `components/store-passport/**` (Store Passport — the topbar dialog describing the selected store: identity/contact, hours, team, facilities, and an access tab of shared logins/door codes masked behind per-row reveal; static data from `lib/mock/store-passport.mock.ts`), `components/break-logger/**` (Breaks — self-service break timer on ToolboxPizza's API via `app/api/toolbox/**` / `TOOLBOX_API_URL`: topbar popover body, `/dashboard/break-logger` Today/History/Settings tabs, entry/notes/delete/export dialogs. `GET breaks/active` and `GET breaks/day` WRITE upstream (milestone evaluation) — never prefetch or cache them), `lib/break-logger/**` (Breaks pure logic: `errors.ts` normalises all four error body shapes into a code-keyed `BreakError`; `work-date.ts` does cutoff/time-zone work-date math — never hardcode 06:00 UTC; `payload.ts` builds changed-keys-only updates, because `ended_at: null` reopens a break), `components/workbooks/**` (Workbooks — ToolboxPizza folders → typed tables → rows, via `app/api/toolbox/**` + `TOOLBOX_API_URL`; every control renders from the server's `viewer.can` and explains refusals from `effective_visibility.capped_by`, never re-deriving the tag rules; a read 404 means "not found or no access"; super-admin only for now), `lib/workbooks/**` (workbook pure logic: error reading, per-type cell values, the whole-list column-replace diff, role list trim/de-dupe, grid query ⇄ URL), `types/**`.
