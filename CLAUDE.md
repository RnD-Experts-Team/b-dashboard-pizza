...
- `app/[locale]/(dashboard)/dashboard/layout.tsx` already gates on `useAuthStore`, shows `<PizzaLoader />` while resolving, and wraps content in `<AppShell>` plus global overlays (`AnnouncementOnLoadPopup`, `FloatingDebriefButton`, `FloatingBreakTimerButton`, `ScreenProjectPiPOverlay`, `DriveThruOverlay`) — new dashboard pages inherit this and don't need to reimplement it.
