...
/**
 * POST /api/v1/stores/[storeId]/hiring-requests/[requestId]/decision
 * Proxy → POST {HIRING_BASE_URL}/v1/stores/{storeId}/hiring-requests/{requestId}/decision
 */
export async function POST(
  ...
  try {
    ...
    const upstream = await fetch(
      `${HIRING_BASE_URL}/v1/stores/${encodeURIComponent(storeId)}/hiring-requests/${encodeURIComponent(requestId)}/decision`,
      {
        method: "POST",
        headers: {
    ...
    console.error("[/api/v1/stores/[storeId]/hiring-requests/[requestId]/decision POST]", err);
