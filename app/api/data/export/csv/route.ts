...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${DATA_BASE_URL}/export/csv?${query.toString()}`;
  ...
  try {
    ...
    if (response.ok) {
      // The upstream likely response with a CSV file stream
      ...
      if (contentDisposition) {
        ...
        headers.set("Content-Disposition", `attachment; filename="export-${model}.csv"`);
      }
