...
async function handleUpstreamError(response: Response) {
  ...
  if (response.status === 404) {
    return errorResponse(
      "NOT_FOUND",
      "No debriefs found for this employee at this store.",
      404
    );
  }
...
export async function GET(
  ...
  if (!employeeId?.trim() || !Number.isInteger(parsedEmployeeId) || parsedEmployeeId <= 0) {
    return errorResponse(
      "VALIDATION_ERROR",
  ...
  const targetUrl = `${DATA_BASE_URL}/stores/${encodeURIComponent(storeId)}/employee-debriefs/employee/${encodeURIComponent(String(parsedEmployeeId))}${queryString ? `?${queryString}` : ""}`;
