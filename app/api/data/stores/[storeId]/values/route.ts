...
async function handleUpstreamError(response: Response) {
  ...
  if (response.status === 404) {
    return errorResponse("NOT_FOUND", "Values were not found for this store.", 404);
  }
...
// Proxies the listing endpoint that (unlike the daily grid/range endpoints) intentionally
// still includes mistaken (superseded) rows, so the client can build a full audit/history view.
// Query params are forwarded as-is — the client narrows further by filtering the response.
export async function GET(
  ...
  const baseUrl = `${DATA_BASE_URL}/engine/stores/${encodeURIComponent(storeId)}/values`;
  const targetUrl = searchParams.toString() ? `${baseUrl}?${searchParams.toString()}` : baseUrl;
