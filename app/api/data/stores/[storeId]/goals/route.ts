...
async function handleUpstreamError(response: Response): Promise<NextResponse> {
  ...
  if (response.status === 404) return errorResponse("NOT_FOUND", "The requested resource was not found.", 404);
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  GET /api/data/stores/[storeId]/goals — list goals for a store          */
/* ────────────────────────────────────────────────────────────────────────── */
...
export async function GET(
  ...
  if (!storeId?.trim()) {
    return errorResponse("MISSING_PARAM", "storeId is required.", 400);
  }

  const url = new URL(request.url);
  const queryString = url.search;
  const targetUrl = `${DATA_BASE_URL}/stores/${encodeURIComponent(storeId)}/goals${queryString}`;
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  POST /api/data/stores/[storeId]/goals — create a goal                  */
/* ────────────────────────────────────────────────────────────────────────── */
...
export async function POST(
  ...
  const targetUrl = `${DATA_BASE_URL}/stores/${encodeURIComponent(storeId)}/goals`;
