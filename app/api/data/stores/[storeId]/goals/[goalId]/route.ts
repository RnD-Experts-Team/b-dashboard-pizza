...
async function handleUpstreamError(response: Response): Promise<NextResponse> {
  ...
  if (response.status === 404) return errorResponse("NOT_FOUND", "The requested goal was not found.", 404);
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  PUT /api/data/stores/[storeId]/goals/[goalId] — update a goal          */
/* ────────────────────────────────────────────────────────────────────────── */

export async function PUT(
  request: NextRequest,
  { params }: { params: Promise<{ storeId: string; goalId: string }> }
) {
  const { storeId, goalId } = await params;

  const authError = requireAuthorization(request);
  if (authError) return authError;

  if (!storeId?.trim()) return errorResponse("MISSING_PARAM", "storeId is required.", 400);
  if (!goalId?.trim()) return errorResponse("MISSING_PARAM", "goalId is required.", 400);

  let body: unknown;
  try { body = await request.json(); }
  catch { return errorResponse("INVALID_PARAM", "Invalid JSON body.", 400); }

  const targetUrl = `${DATA_BASE_URL}/stores/${encodeURIComponent(storeId)}/goals/${encodeURIComponent(goalId)}`;
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  DELETE /api/data/stores/[storeId]/goals/[goalId] — delete a goal       */
/* ────────────────────────────────────────────────────────────────────────── */
