...
async function handleUpstreamError(response: Response) {
  ...
  if (response.status === 403) {
    return errorResponse("FORBIDDEN", "You do not have permission to restore this key.", 403);
  }
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  PATCH /api/data/keys/[id]/restore — restore (reactivate) a key          */
/* ────────────────────────────────────────────────────────────────────────── */
...
export async function PATCH(
  ...
  const targetUrl = `${DATA_BASE_URL}/engine/keys/${id}/restore`;
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Some restore endpoints return 204 No Content
      if (!body) {
