...
async function handleUpstreamResponse(response: Response, action: string) {
  ...
  if (response.status === 403) {
    return errorResponse(
      "FORBIDDEN",
      `You do not have permission to ${action} custom reports.`,
      403
    );
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return handleUpstreamResponse(response, "list");
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return handleUpstreamResponse(response, "create");
  } catch (error) {
    return handleUpstreamError(error);
  }
}
