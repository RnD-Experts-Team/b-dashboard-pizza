...
export function TaskDetailSheet({ taskId, open, onOpenChange, onEdit, onDelete }: Props) {
  ...
  useEffect(() => {
    ...
    cleaningService
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof CleaningError ? err.message : "Failed to load task.");
      })
            ...
            <div className="min-w-0 flex-1">
              ...
              {task && (
                <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                  <Badge variant="secondary">{FREQ_LABEL[task.frequency] ?? task.frequency}</Badge>
                  <Badge variant={task.photoRequired ? "default" : "outline"}>
                    <Camera className="me-1 h-3 w-3" />
                    {task.photoRequired ? "Photo required" : "Photo optional"}
                  </Badge>
        ...
        <div className="px-6 py-5">
          {loading && (
            <div className="flex items-center justify-center py-16 text-muted-foreground">
              <Loader2 className="me-2 h-5 w-5 animate-spin" /> Loading…
            </div>
          )}
          ...
          {task && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field icon={<Repeat className="h-3.5 w-3.5" />} label="Frequency">
                ...
                {task.frequency === "hourly" ? (
                  <Field icon={<Clock className="h-3.5 w-3.5" />} label="Interval">
                    {task.intervalHours ? `Every ${task.intervalHours}h` : "—"}
                  </Field>
                ) : (
                  <Field icon={<Weight className="h-3.5 w-3.5" />} label="Weight">
                    {task.weight ?? "—"}
                  </Field>
                )}

                <Field icon={<CalendarClock className="h-3.5 w-3.5" />} label="Starts on">
                  {formatDate(task.startsAt)}
                </Field>

                <Field icon={<CalendarClock className="h-3.5 w-3.5" />} label="Ends on">
                  {task.endsAt ? (
                    formatDate(task.endsAt)
                  ) : (
                    <span className="font-normal text-muted-foreground">Repeats forever</span>
                  )}
                </Field>
                ...
                {task.dueTime && (
                  <Field icon={<Clock className="h-3.5 w-3.5" />} label="Due time">
                    {task.dueTime}
                  </Field>
                )}
              </div>
              ...
              <div className="space-y-2">
                <p className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  <StoreIcon className="h-3.5 w-3.5" /> Assigned stores ({task.stores.length})
                </p>
