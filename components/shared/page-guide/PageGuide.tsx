...
export function PageGuide({ steps, isOpen, onClose }: PageGuideProps) {
  ...
  // Keyboard: Esc close, â†â†’ navigate
  useEffect(() => {
    if (!isOpen) return;
  ...
  const cardPos = (() => {
    ...
    // Intro / noHighlight: center on screen
    if (isIntro) {
      return {
        left: Math.max(PAD, (vp.w - INTRO_CARD_W) / 2),
        top: Math.max(PAD, vp.h * 0.2),
        width: INTRO_CARD_W,
      };
    }

    // Normal step: position relative to target
    if (!targetRect) return null;
    const cx = targetRect.left + targetRect.width / 2;
    let left = cx - CARD_W / 2;
    let top: number;

    if (targetRect.bottom + GAP + CARD_H_EST <= vp.h - PAD) {
      top = targetRect.bottom + GAP;
    } else if (targetRect.top - GAP - CARD_H_EST >= PAD) {
      top = targetRect.top - GAP - CARD_H_EST;
    } else {
      top = vp.h - CARD_H_EST - PAD * 2;
    }

    left = Math.max(PAD, Math.min(left, vp.w - CARD_W - PAD));
